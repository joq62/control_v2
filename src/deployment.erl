...
delete_application(AppSpec)->
    Result=case if_db:call(db_sd,app_spec,[AppSpec]) of
	       []->
...
missing_apps()->
    ...
    ActiveServicesApps=if_db:call(db_sd,active_apps,[]),
    WantedApps=if_db:call(db_app_spec,all_app_specs,[]),
...
get_wanted_service_info(AppSpec,_AppVsn,Directive,ServiceSpecs)->
   ...
   % io:format("ServiceSpecs ~p~n",[ServicSpecs]),
    ...
    ServiceInfo=lists:append([if_db:call(db_service_def,read,[ServiceSpec])||ServiceSpec<-ServiceSpecs]),
